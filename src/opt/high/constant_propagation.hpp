...
inline CP::CP(Module *_m) : ctx(&_m->ctx), m(_m) {}

inline auto CP::run(Function &f, FunctionAnalysisManager & /* FAM */)
  -> PreservedAnalysis {
  env.clear();
  safe_allocas.clear();
  safe_globals.clear();
...
inline auto CP::visit(Op *op) -> void {
...
inline auto CP::ModifiedFinder::visit(Op *op, OpTag<OpCode::Store>) -> void {
  modified.insert(op->operands[1]);
}

inline auto CP::ModifiedFinder::visit(Op *op, OpTag<OpCode::Memset>) -> void {
  modified.insert(op->operands[0]);
}

inline auto CP::ModifiedFinder::visit(Op * /* op */, OpTag<OpCode::Call>)
  -> void {
  has_call = true;
}

inline auto CP::visit(Op *op, OpTag<OpCode::Alloca>) -> void {
...
inline auto CP::visit(Op *op, OpTag<OpCode::Load>) -> void {
...
inline auto CP::visit(Op *op, OpTag<OpCode::Store>) -> void {
...
inline auto CP::visit(Op *op, OpTag<OpCode::Call>) -> void {
  RecursiveOpVisitor<CP>::visit(op, OpTag<OpCode::Call>{});
  clear_global_env();
}

inline auto CP::visit(Op *op, OpTag<OpCode::If>) -> void {
  auto &p = std::get<IfPayload>(op->payload);
...
inline auto CP::visit(Op *op, OpTag<OpCode::While>) -> void {
...
inline auto CP::clear_global_env() -> void {
...
inline auto CP::clear_all_env() -> void { env.clear(); }

template <typename T>
inline auto CP::fold_arith(OpCode code, T l, T r)
...
inline auto CP::try_fold(Op *op) -> void {

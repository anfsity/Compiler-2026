...
inline auto IRPrinterBase::get_value_name(const high_ir::Value *v)
  -> std::string {
...
inline auto IRPrinterBase::dump_global(const high_ir::GlobalVar &v)
  -> std::string {
  return fmt::format(
    "{} = global {} : {}\n",
    v.addr->dump(),
...
inline auto IRPrinterBase::dump_init(const high_ir::InitVal &i) -> std::string {
  return std::visit(
    overload{
      [](int val) -> std::string { return std::to_string(val); },
      [](float f) -> std::string { return std::to_string(f) + "f"; },
      [](high_ir::ZeroInit) -> std::string { return std::string("zeroinit"); },
...
inline auto IRPrinterBase::opcode_to_str(high_ir::OpCode oc) -> std::string {
  switch (oc) {
    // clang-format off
...
inline auto IRPrinterBase::dump_op_common(const high_ir::Op &op)
  -> std::string {
  std::string line;
...
inline auto
IRPrinterBase::join_operands(const std::vector<high_ir::Value *> &ops)
  -> std::string {
  std::string res;
...
inline auto
IRPrinterBase::join_operand_types(const std::vector<high_ir::Value *> &ops)
  -> std::string {
  std::string res;
  for (size_t i = 0; i < ops.size(); ++i) {

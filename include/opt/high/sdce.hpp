...
inline SimpleDCE::SimpleDCE(Module * /* m */) {}

inline auto SimpleDCE::isIntrinsicallyLive(Op *op) -> bool {
  switch (op->code) {
  case OpCode::Ret:
  case OpCode::Call:
  case OpCode::Break:
...
inline auto SimpleDCE::buildParentMap(Region &r, Op *parent) -> void {
  for (auto *op : r) {
    parents[op] = parent;
...
inline auto SimpleDCE::mark(Op *op) -> void {
  if (op && liveset.insert(op).second) {
    worklist.push_back(op);
  }
}

inline auto SimpleDCE::initialMark(Region &r) -> void {
  for (auto *op : r) {
...
inline auto SimpleDCE::collectDead(Region &r) -> void {
  for (auto *op : r) {
    if (!liveset.count(op)) {
      rewriter.eraseOp(op);
    }
...
inline auto SimpleDCE::run(Function &f, FunctionAnalysisManager & /* FAM */)
  -> PreservedAnalysis {
  if (f.is_decl)

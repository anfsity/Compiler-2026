...
inline Inliner::Inliner(Module *_m) : m(_m) {
  for (auto &f : m->functions) {
    func_map[f->name] = f.get();
  }
}

inline auto Inliner::run(Module &, ModuleAnalysisManager &)
  -> PreservedAnalysis {
  call_graph.build(*m);
...
inline auto Inliner::getRetStatus(const Region &r) -> RetStatus {
...
inline auto Inliner::runOnFunction(Function &f) -> bool {
  bool changed = false;
...
inline auto Inliner::tryInlineInRegion(Region &r, Function &caller, int depth)
  -> bool {
  for (auto it = r.begin(); it != r.end(); ++it) {
    Op *op = *it;
...
inline auto
Inliner::shouldInline(Op &call_op, Function &callee, Function &, int depth)
  -> bool {
  if (callee.is_decl)
    return false;
...
inline auto Inliner::inlineCall(
